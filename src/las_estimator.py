...
def data_input_fn():
    ...
    #         sparse_indices=feature.indices,
    #         sparse_values=feature.values,
    #         output_shape=feature.dense_shape
    #     )
...
def model_fn(features, labels, mode, params):
    ...
    network.create_graph(use_tfrecords=True,
                         ...
                         labels_tensor=features['sparse_target'],
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
        ...
        # return tf.estimator.EstimatorSpec(mode, predictions=tf.constant(1.0, dtype=tf.float32))
        logging_hook = tf.train.LoggingTensorHook({
            "Predicted": dense_decoded#ClassicLabel.from_index(dense_decoded),
...
model = tf.estimator.Estimator(
    model_fn=model_fn,
    params={'hola': 2},
    model_dir='out/zorznet/estimator/'#'./test_estimator/'
)
tf.logging.set_verbosity(tf.logging.INFO)
# model.train(input_fn=data_input_fn, steps=2)

prediction = model.predict(input_fn=data_input_fn)
for item in prediction:
    print(ClassicLabel.from_index(item))
# model.evaluate(input_fn=data_input_fn, )